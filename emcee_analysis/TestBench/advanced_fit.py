...
eff_M = np.load('eff_matrix_15Corr.npy')
raw_dp_acc = np.load('raw_DP_Acc.npy')
...
#dalitz_data, raw_smear_M = rebin_data(raw_dalitz_data,dp_acc)



global_bin = raw_dalitz_data[:,6]

N_true = raw_dalitz_data[:,2]
DN_true = raw_dalitz_data[:,3]

N_rec = raw_dalitz_data[:,4]
DN_rec = raw_dalitz_data[:,5]

eff = N_rec / (N_true + 1e-5)
D_eff = get_efficiency_error(N_rec,N_true,1e-8)

N_eta = np.where(eff>0.0,N_rec/eff,0.0)

DN_eta = np.sqrt(DN_rec*DN_rec + (N_eta * D_eff)**2) / eff

plt.rcParams.update({'font.size': 20})

figm,axm = plt.subplots(figsize=(10,8))

axm.matshow(eff_M,norm=LogNorm())
axm.set_xlabel('Reconstructed Global Bin')
axm.set_ylabel('True Global Bin')

plt.show()

figa, axa = plt.subplots(figsize=(10,8))

axa.plot(global_bin,raw_dp_acc,'ks',markersize=10)
axa.set_xlabel('Global Bin')
axa.set_ylabel('Kinematic Acceptance')
axa.grid(True)
...
ax[0].errorbar(x=global_bin,y=N_true,yerr=DN_true,fmt='rd',markersize=10)
ax[0].set_ylabel('True ' + r'$\eta\rightarrow\pi^+\pi^-\pi^0$' + ' Yields')
ax[0].grid(True)

ax[1].errorbar(x=global_bin,y=N_rec,yerr=DN_rec,fmt='ko',markersize=10)
ax[1].set_ylabel('Reconstructed ' + r'$\eta\rightarrow\pi^+\pi^-\pi^0$' + ' Yields')
ax[1].grid(True)

ax[2].errorbar(x=global_bin,y=eff,yerr=D_eff,fmt='bs',markersize=10)
ax[2].set_ylim(0.0,1.0)
ax[2].set_ylabel('Efficiency')
ax[2].grid(True)

ax[0].set_xlabel('Global Bin')
ax[1].set_xlabel('Global Bin')
ax[2].set_xlabel('Global Bin')
...
def get_dp_acceptance(x):
    ...
    return np.where(raw_dp_acc>=f,1.0,0.0)
...
minDPVals = [100000,-1.2,0.05,-0.01,0.05,-0.01,0.1,-0.01,-0.01,-0.01,-2.0]
maxDPVals = [200000,-1.0,0.2,0.01,0.1,0.01,0.2,0.01,0.01,0.01,0.01,2.0]
...
fit_par_idx = [i for i in range(nPars-2)]


nwalkers, ndim = pos.shape

sampler = emcee.EnsembleSampler(
    nwalkers, ndim, log_probability, args=(raw_dalitz_data[:,[0,1]],N_eta,DN_eta,c_err)
)
...
# print("log_f: " + str(fit_pars[10]))

N_fit = oneD_Dalitz_Function(fit_pars,raw_dalitz_data[:,[0,1]])

fit_acc = get_dp_acceptance(fit_pars[10])
...
axf[1].errorbar(x=fit_par_idx,y=fit_pars[1:nPars-1],yerr=fit_pars_errs[1:nPars-1],fmt='rs',label='Fit',markersize=10.0)
